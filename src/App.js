...
const plugins = [...pluginsBasic, createFieldPlugin()];
...
function App() {
  ...
  const handleAppDataChange = (op, nodes, pageId = "") => {
    ...
    if (op === "U") {
      newPagesData = pagesData.map((pageData) =>
        pageData.selected ? { ...pageData, nodes: nodes } : pageData
      );
    } else if (op === "C") {
      newPagesData = [
        ...pagesData,
        ...
        <Row>
          ...
          <Col lg>
            <div className="App">
              ...
              <Plate
                id={pagesData.filter((pageData) => pageData.selected)[0].pageId}
                editableProps={editableProps}
                initialValue={
                  pagesData.filter((pageData) => pageData.selected)[0].nodes
                }
                value={
                  pagesData.filter((pageData) => pageData.selected)[0].nodes
                }
